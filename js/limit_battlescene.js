 ...
 * 游戏主函数
 */
export default class Limit_BattleScene {
  constructor(e) {
    ...
    this.initEvent()
    this.playerA = new actorimp(0, usedata, true)
  ...
  checkskill(x, y) {
    if (this.stopflag == false) {
      if (this.result == 0) {
        ...
        scenemanager.stoplimitballtescene()
        ...
        if (this.result != 1) {
          ...
          if (scenemanager.haslimit()) {
            scenemanager.restartlimit(usedata.mapid, true)
          } else {
            var p = new LimitScene(usedata.mapid, true)
