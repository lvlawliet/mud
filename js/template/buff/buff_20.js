...
export default class buff_20 extends buff_template {
  constructor() {
    super()
  }
  ...
  doeffectonroundend(cast, target) {
    ...
    damage = damage * cast.buffs[20].number
    // wuxing
    var percent = 1
    for (var i = 0; i < cast.wuxing.length; i++) {
      percent = percent * tempman.wuxingrestrain[skilldata.buffs[20].wuxing][cast.wuxing[i]]
    }
    ...
    cast.hpadd(-damage)
    dinfo.push(target.name + "的[" + skilldata.buffs[20].name + "]对" + cast.name + "造成了" + damage + "点伤害")
    return dinfo
  }
